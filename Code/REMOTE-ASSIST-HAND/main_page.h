...
<html>
    <head>
        <!-- Page title and responsive viewport -->
    ...
    <body>
        ...
        <!-- Container for all motor control cards -->
        <div class="content">
            <!-- Force reading display -->
            <div id="force-display">Force: 0 N</div>
            <div class="card-grid">
                <!-- Base Stepper motor controls -->
                <div class="card">
        ...
        <script>
            ...
            let websocket;
            // Store intervals for continuous commands
            const intervals = {};
            ...
            // Handle incoming messages (e.g., telemetry)
            function onMessage(event) {
                try {
                    const data = JSON.parse(event.data);
                    if (data.type === 'force' && data.motor === 'grasper_servo') {
                        // Update force display in N
                        const forceEl = document.getElementById('force-display');
                        forceEl.textContent = `Force: ${data.force.toFixed(2)} N`;
                    } else {
            ...
            // Send a single motor command over WebSocket
            ...
            // Start continuous movement: send initial command and then repeat every 200ms
            ...
            // Stop continuous movement and optionally send a stop message
